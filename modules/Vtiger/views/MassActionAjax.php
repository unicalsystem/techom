...
class Vtiger_MassActionAjax_View extends Vtiger_IndexAjax_View {
	...
	function showSendSMSForm(Vtiger_Request $request) {
        ...
        // Fetch and assign phone numbers
        $phoneNumbers = $this->fetchPhoneNumbers($selectedIds);
        $viewer->assign('PHONE_NUMBERS', $phoneNumbers);
    
        // Assign the selected IDs as JSON for potential use in JavaScript
        $selectedIdsJson = json_encode($selectedIds);
        $viewer->assign('SELECTED_IDS_JSON', $selectedIdsJson);
    
        echo $viewer->view('SendSMSForm.tpl', $moduleName, true);
    }

	//Function to fetch the phone number for the selected records
	function fetchPhoneNumbers($selectedIds) {
        global $adb; 
        ...
        $query = "SELECT phone FROM vtiger_leadaddress WHERE leadaddressid IN ($idList)";
        ...
        //echo "<script>console.log('Phone Numbers: " . json_encode($phoneNumbers) . "');</script>";
