...
class CTBrowserExt_WS_SyncModuleRecords extends CTBrowserExt_WS_SaveRecord {

	static $SYNC_MODE_PUBLIC  = "PUBLIC";
	static $SYNC_MODE_PRIVATE = "PRIVATE";

	function isModePrivate(CTBrowserExt_API_Request $request, $defmode="PRIVATE") {
		return (strcasecmp($request->get('mode', $defmode), self::$SYNC_MODE_PRIVATE) === 0);
	}

	function process(CTBrowserExt_API_Request $request) {
		$current_user = $this->getActiveUser();
		$current_user_wsid = sprintf("%sx%s", CTBrowserExt_WS_Utils::getEntityModuleWSId("Users"), $current_user->id);
		...
		if (!empty($lastSyncTime) && $currentPage === 0) {
			...
			if ($hasAssignedToField) {
				if ($isPrivateMode) {
					...
					$andsmowneridequal = CTBrowserExt_WS_Utils::querySecurityFromSuffix($module, $current_user);
				}
			}
