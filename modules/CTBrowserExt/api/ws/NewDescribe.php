...
class CTBrowserExt_WS_NewDescribe extends CTBrowserExt_WS_FetchRecord {
			...
			if(!empty($record)){
				...
				if(isset($resultRecord[$fieldname])){
					if(in_array($field['uitype'], $refrenceUitypes)){
						$field['value'] = $resultRecord[$fieldname]['value'];
					}else if(in_array($field['uitype'], array('5','6','23'))){
						$field['value'] = Vtiger_Date_UIType::getDisplayDateValue($resultRecord[$fieldname]);
