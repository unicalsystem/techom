...
class CTBrowserExt_WS_Utils {
	...
	static function gatherModuleFieldGroupInfo($module) {
		...
		$result = $adb->pquery(
			"SELECT fieldname, fieldlabel, blocklabel, uitype, summaryfield, typeofdata FROM vtiger_field INNER JOIN
		...
		while($resultrow = $adb->fetch_array($result)) {
			...
			$fieldgroups[$blocklabel][$resultrow['fieldname']] = 
				array(
					...
					'summaryfield' => $resultrow['summaryfield'],
	...
	static function getModuleURL($modulename) {
		global $adb,$site_URL;
		$img_url = '';	
