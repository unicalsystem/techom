...
class CTBrowserExt_WS_RelatedSearchList extends CTBrowserExt_WS_Controller {
	public $totalQuery = "";
	...
	function processSearchRecordLabel(CTBrowserExt_API_Request $request) {
		...
		foreach ($modifiedRecords as $key => $part) {
			foreach($part as $keys => $value){
				if(!in_array($keys,array('id','label'))){
		...
		$response = new CTBrowserExt_API_Response();
		if(count($modifiedRecords) == 0) {
			$response->setResult(array('records'=>$modifiedRecords, 'module'=>$module,'moduleLabel'=>$moduleLabel, 'message'=>vtranslate('LBL_NO_RECORDS_FOUND','Vtiger'),'module_record_status'=>false,'createAction'=>$createAction,'entity_key'=>$entity_key,'isLast'=>$isLast));
		} else {
			$response->setResult(array('records'=>$modifiedRecords, 'module'=>$module,'moduleLabel'=>$moduleLabel, 'message'=>'','module_record_status'=>true,'createAction'=>$createAction,'entity_key'=>$entity_key,'isLast'=>$isLast));
		}
	...
	function processSearchRecordLabelForCalendar(CTBrowserExt_API_Request $request,$filterOrAlertInstance = false, $pagingModel = false, $paging = array(),$field_name, $field_value,$order_by,$orderby,$related,$params) {
		...
		$response = new CTBrowserExt_API_Response();
		if(count($modifiedRecords) == 0) {
			$response->setResult(array('records'=>$modifiedRecords, 'module'=>$module,'moduleLabel'=>$moduleLabel, 'message'=>vtranslate('LBL_NO_RECORDS_FOUND','Vtiger'),'module_record_status'=>false,'isLast'=>$isLast));
		} else {
			$response->setResult(array('records'=>$modifiedRecords, 'module'=>$module,'moduleLabel'=>$moduleLabel, 'message'=>'','module_record_status'=>true,'isLast'=>$isLast));
		}
