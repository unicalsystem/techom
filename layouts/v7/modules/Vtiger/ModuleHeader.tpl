									...
									{if $MODULE_SETTING_ACTIONS|@count gt 0}
										<li>
											<div class="settingsIcon">
                                                ...
                                                {assign var=FILTERED_ACTIONS value=[]}
                                                {foreach item=SETTING from=$MODULE_SETTING_ACTIONS name=moduleSettings}
                                                    {assign var=SETTING_INDEX value=$smarty.foreach.moduleSettings.iteration}
                                                    {if
                                                        ($MODULE eq 'Accounts' && $SETTING_INDEX neq 4) ||
                                                        ($MODULE eq 'Contacts' && $SETTING_INDEX neq 4) ||
                                                        ($MODULE eq 'Potentials' && $SETTING_INDEX neq 4 && $SETTING_INDEX neq 6) ||
                                                        ($MODULE eq 'Leads' && $SETTING_INDEX neq 4 && $SETTING_INDEX neq 6) ||
                                                        ($MODULE neq 'Accounts' && $MODULE neq 'Contacts' && $MODULE neq 'Potentials' && $MODULE neq 'Leads')
                                                    }
                                                        {append var=FILTERED_ACTIONS value=$SETTING}
                                                    {/if}
                                                {/foreach}
                                               
                                                {foreach item=SETTING from=$FILTERED_ACTIONS}
