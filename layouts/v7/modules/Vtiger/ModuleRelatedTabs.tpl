        ...
        <div class="collapse navbar-collapse" id="nav-tabs">
            <ul class="nav nav-tabs">
                {foreach item=RELATED_LINK from=$DETAILVIEW_LINKS['DETAILVIEWTAB']}
                    ...
                    <li class="tab-item {if $RELATED_TAB_LABEL==$SELECTED_TAB_LABEL}active{/if}" 
                        data-url="{$RELATEDLINK_URL}&tab_label={$RELATED_TAB_LABEL}&app={$SELECTED_MENU_CATEGORY}" 
                        data-label-key="{$RELATEDLINK_LABEL}" 
                        data-link-key="{$RELATED_LINK->get('linkKey')}"
                        {if $RELATEDLINK_LABEL eq 'LBL_UPDATES'}style="margin-left: 10px; margin-right: 26px;"{/if}>
                        <a href="{$RELATEDLINK_URL}&tab_label={$RELATEDLINK_LABEL}&app={$SELECTED_MENU_CATEGORY}" 
                           class="textOverflowEllipsis"
                           {if $RELATEDLINK_LABEL eq 'LBL_UPDATES'}style="margin-left: -13px;"{/if}>
                                            ...
                                            {for $i = 0 to $COUNT1-1}
                                                    ...
                                                    <li class="tab-item {if (trim($RELATED_LINK->getLabel())== trim($SELECTED_TAB_LABEL)) && ($RELATED_LINK->getId() == $SELECTED_RELATION_ID)}active{/if}" data-url="{$RELATED_LINK->getUrl()}&tab_label={$RELATED_LINK->getLabel()}&app={$SELECTED_MENU_CATEGORY}" data-label-key="{$RELATED_LINK->getLabel()}"
                                                            ...
                                                            <a href="index.php?{$RELATED_LINK->getUrl()}&tab_label={$RELATED_LINK->getLabel()}&app={$SELECTED_MENU_CATEGORY}" class="textOverflowEllipsis" displaylabel="{$DETAILVIEWRELATEDLINKLBL}" recordsCount="" >
                                                                    ...
                                                                    {else}
                                                                             <span class="tab-icon">
                                        {if $RELATEDMODULENAME eq "Accounts" || $RELATEDMODULENAME eq "Contacts" || $RELATEDMODULENAME eq "Leads" || $RELATEDMODULENAME eq "Potentials"}
                                            <img src="layouts/v7/skins/images/icons/{$RELATEDMODULENAME}.png" alt="{$RELATEDMODULENAME}" style="width: 24px; height: 24px; color: #666666">
                                        {else}
