...
function fetchTemplates(bearerToken) {
...
loginAndFetchToken().then(bearerToken => {
    if (bearerToken) {
        fetchTemplates(bearerToken).then(fetchedTemplates => {
        ...
        document.querySelector('button[name="sendWhatsappButton"]').addEventListener('click', () => sendMessage(bearerToken));
    }
});
...
function sendMessage(bearerToken) {
    if (isSending) return; // Prevent further execution if already sending
    isSending = true;
